...
RUN cd /ukwa-manage && pip install --no-cache-dir https://github.com/ukwa/hapy/archive/master.zip && pip install --no-cache-dir -r requirements.txt
...
# Run the dashboard:
CMD gunicorn --error-logfile - --access-logfile - --bind 0.0.0.0:8000 --workers 2 dash.dashboard:app

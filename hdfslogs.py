...
if __name__ == "__main__":
	w = webhdfs.API( prefix="http://%s:14000/webhdfs/v1" % settings.hdfshost, user=settings.hdfsloguser )
	for path in settings.logdirectories.split( "," ):
		...
		if not path.endswith( "/" ):
			path = "%s/" % path
		hdfs_prefix = "%s/%s" % ( settings.hdfslogroot, gethostname() )

...
# HTTP timeout in seconds (to avoid hangs)
TIMEOUT = 60
...
class DocumentMDEx(object):
    ...
    def mdex_default(self):
        ...
        r = requests.get(self.lp_wb_url(), stream=True, verify=False, timeout=TIMEOUT)
    ...
    def mdex_gov_uk_publications(self):
        ...
        success = False
        while tries > 0:
            r = requests.head(url=self.doc_wb_url(), allow_redirects=True, timeout=TIMEOUT)
            if 'up' in r.links:
                lpu = r.links['up']
        ...
        if "www.gov.uk" == lp_url.hostname:
            ...
            if not lp_url.path.startswith('/government/organisations/'):
                ...
                r = requests.get(api_json_url, timeout=TIMEOUT)
            ...
            r = requests.get(self.lp_wb_url(), timeout=TIMEOUT)
            if r.status_code != 200:

...
class ScanLogForDocs(luigi.Task):
    ...
    def run(self):
        ...
        with open(self.path, 'r') as f:
            for line in f:
                self.process_document(line, watched)

    def process_document(self, line, watched):
        ...
        if "application/pdf" in mime:
            for prefix in watched:
                if url.startswith(prefix):
                    ...
                    if self.document_available(systems().wayback, doc['document_url'], doc['wayback_timestamp']):
    ...
    def document_available(self, wayback_url, url, ts):
        """

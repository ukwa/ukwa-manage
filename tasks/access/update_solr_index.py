		...
		if listsize.st_size == 0:
			...
			if not self.solr_index_verify_mark_fail:
				...
				with open(self.input().path, 'r') as wlh:
					for self.warc_line in wlh:
						self.warc_num += 1
						logger.info("---- {}) Verifying WARC {}".format(self.warc_num, self.warc_line))
						# Verify WARC in Solr
						solr_verify_task = SolrVerify(warc=self.warc_line, solr_api=self.solr_api)
						...
						if not solr_verify_task.complete():
							logger.error("==== Solr verify of {warc} run into {stream}-{year} failed {now}".format(
								now=datetime.datetime.now(), stream=self.stream, year=self.year, warc=self.warc_line))
							self.solr_index_verify_mark_fail = True
						else:
							# Mark WARCs in trackdb as indexed into Solr
							logger.info("---- Marking WARC as indexed {}".format(self.warc_line))
							mark_warc_task = MarkWarc(warc=self.warc_line, tracking_db_url=self.tracking_db_url,
